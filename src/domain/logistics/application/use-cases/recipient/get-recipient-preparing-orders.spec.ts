...
import { GetRecipientPreparingOrdersUseCase } from "./get-recipient-preparing-orders"
import { BadRequestError } from "@/core/errors/bad-request-error"

let recipientsRepository: InMemoryRecipientsRepository
let sut: GetRecipientPreparingOrdersUseCase

describe("Get Recipient Preparing Orders Use Case", async () => {
  beforeEach(async () => {
    recipientsRepository = new InMemoryRecipientsRepository()
    sut = new GetRecipientPreparingOrdersUseCase(recipientsRepository)
  })

  it("should get recipient preparing orders", async () => {
    ...
    expect(result.value).toEqual(expect.objectContaining({
      items: expect.arrayContaining([expect.objectContaining({ status: OrderStatus.PREPARING })]),
    }))
  })

  it("should get recipient preparing orders with pagination", async () => {
